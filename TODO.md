...
5. ℹ️ **Strategic direction** - pipetint is a unique CLI filter for regex-based highlights with nested priority rules
6. ✅ **--unbuffered CLI flag** - Added `-u`/`--unbuffered` for line-buffered output (PR #7)

# Technical Debt

- `sre_parse` is deprecated (Python 3.11+). Monitor for removal in future Python versions; may need to migrate to a third-party regex parsing library. See `src/pipetint/core.py:10-15`.
